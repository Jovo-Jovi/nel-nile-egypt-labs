...
--
-- Reverse of 20260831104408_m3_catalogue_tables.sql. Drop order is the reverse of
...
--
--   "ProgrammeTier" — one inbound foreign key, from "ProgrammeLabTest", dropped
--   above. §6 gives it no other. It holds no rows.
--
--   "Programme" — one inbound foreign key, from "ProgrammeTier", dropped above.
--   There is no key from "ProgrammeLabTest" to "Programme" (D-44). CONTENT_MODEL.md
...
-- 20260831104408_m3_catalogue_tables.sql or accept the exposure deliberately.
--
...
-- statement. pg_default_acl carries two granting roles — postgres and
-- supabase_admin — so a single statement restores only one.
