...
## 2026-09-25 — P07-T07: the G7 carry-forward triage

The human signed the G7 triage on 25 September 2026, approving every disposition as proposed, and set four clarifications. The Supabase MCP connection stays active and unchanged until the final transfer step. The repository keeps its current visibility until the human explicitly authorises that step. The pending PartnerLab account stays as it is; the Operator decides it through the existing queue. The finished website is approved, and from here only documentation, scope descriptions and strictly necessary security work proceed.

The signed file covers the 109 rows that were open at P07-T06. Thirty-one close as already resolved or superseded. Two close by documentation corrections in this task. Forty-two are accepted as delivered. Twenty-two move to maintenance. Twelve stay open as handover items. The apply script then allocated CF-224 and CF-225. CF-224 accepts the pending PartnerLab account and leaves it for the Operator. CF-225 corrects the quotation's A2 row, which had called the vector mark delivered. After those two rows the ledger, counted by python re.findall on `docs/method/CARRY_FORWARDS.md`, is 225 rows, maximum 225, duplicates 0, header CF-226, with 12 OPEN, 43 ACCEPTED at P07-T07, 22 MAINTENANCE and 34 CLOSED at P07-T07. The STEP 0 base was 223 rows, header CF-224 and 109 OPEN.

The twelve handover rows and the steps that close them: CF-07 at Stage 5, when the DNS inventory settles who controls `nileegyptlabs.org`. CF-28 and CF-213 at Stage 4, the final transfer, when the human authorises making the repository private. CF-37 at Stage 4, on the plan tier, pausing and backups. CF-49 at G7, on the laboratory's written approval of the mark. CF-105 at Stage 3, named in the handover document as the laboratory's own PDPL obligations. CF-117 and CF-119 at Stage 5, when the post-cutover checks verify that the portal URL renders. CF-118 at Stage 5, the laboratory's portal certificate, a pre-cutover check. CF-126 at G7, the client's countersignature of OD-16. CF-166 at Stage 3, where the handover document states the hosted password policy and that it must not be lowered. CF-203 at Stage 4, the final transfer, when the builder environment's Supabase MCP connection is revoked. This task does not revoke that connection, does not change the repository's visibility, and does not touch the pending PartnerLab account.

## 2026-09-25 — P07-T08: the handover documents

Stage 3 of OD-35 landed four reviewer-authored documents byte-exact: `docs/CUTOVER_RUNBOOK.md`, `docs/HANDOVER.md`, `docs/ADMIN_GUIDE.md` and `docs/commercial/maintenance-agreement-draft.md`. The commercial index gained a row for the maintenance agreement, and the repository README gained two lines pointing at the handover, the cutover plan and the Operators' guide. Placeholders in those documents are unfilled. P07-T07 is recorded PASS at reviewer verdict on 25 September 2026, and its checklist box is checked.

The first issue of this task halted correctly under PR-25. It was told to escape three pipes in the P07-T07 done-step row, and those pipes were already escaped. The phases guard counts a pipe even when a backslash precedes it, so that escape could not change the report-only list. The reviewer's P07-T07 finding was mistaken. The finding is withdrawn, and no carry-forward was landed. The ledger is unchanged.

The Supabase MCP connection stays active. The repository keeps its current visibility. The pending PartnerLab account was not touched.
