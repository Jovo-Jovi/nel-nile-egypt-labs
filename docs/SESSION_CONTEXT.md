...
Computed by (run after STEP 8 of P02-T13):
`grep -cE '^\| CF-[0-9]+ .*\| OPEN \|' docs/method/CARRY_FORWARDS.md`

**Open — 44:** CF-01 · CF-03 · CF-04 · CF-05 · CF-06 · CF-07 · CF-08 · CF-09 ·
...
CF-66 · CF-67 · CF-68 · CF-69 · CF-70 · CF-71
...
**The client preview review.** The landing mock in `src/` now renders against
`DESIGN_SYSTEM.md` v4, landed at P02-T13: the page wash, the crafted `pending`
treatment on every gated region, card hover and hover-reveal, the drawn
Greater Cairo map (itself `pending` on `Branch` coordinates), and both
WhatsApp treatments are all live at `/`, both locales, one route, no new
scope. This task touched only `docs/method/CARRY_FORWARDS.md`, this file and
`src/` — `DESIGN_SYSTEM.md`, `DECISIONS.md`, `I18N_MODEL.md`,
`CONTENT_MODEL.md`, `BOUNDARY_MODEL.md` and `GLOSSARY.md` are all untouched.
Nothing further in P02 is buildable without client input: the preview is
ready to be screenshotted or opened for Androw's review, and every open
item below is a signature, a supplied asset or a verbal approval getting
filed — not a design task.
...
time, never on a design task (CF-67, CF-68, CF-71).** Of the preview's 12
`pending` regions: the mark (header and footer) clears the moment
...
the Locations card's drawn map clears on verified `Branch` coordinates
(CF-69); the hero media and the three Video cards clear on client-supplied
photography and video assets. No further P02 task changes any of these —
each is a signature or an asset, named in CF-71.
