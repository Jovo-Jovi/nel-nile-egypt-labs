...
| Calling, scraping or authenticating against the results portal | Outbound `https://` link only |
...
no support contract in place. Tracked as CF-11. Not our system; a failure there
...
With no forms and no cookies, a cookie-consent banner may not be required at all.
Confirm with the client's legal advisor before assuming either way — but note the
simplification is real and worth the question.
