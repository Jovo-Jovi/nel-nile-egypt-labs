...
`scripts/audit/` holds evidence tooling that measures the repository or the database for a task's report. `scripts/audit/inventory.py`, landed at P07-T03, prints every count `docs/SCOPE.md` and `docs/PRODUCT_BRIEF.md` state; run it from the repository root with `python -X utf8 scripts/audit/inventory.py`.
