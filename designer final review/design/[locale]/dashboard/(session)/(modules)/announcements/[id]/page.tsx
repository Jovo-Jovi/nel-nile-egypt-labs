...
import { isRowId, readAnnouncementRow } from "@/lib/dashboard/catalogEntities";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return pageMetadata(locale, "dashboard.announcements.heading", "/dashboard/announcements");
}

export default async function AnnouncementEditPage({ params, searchParams }: Props) {
  ...
  if (supabase === null) {
    return (
      <>
        <DashboardModuleTitle locale={locale} titleKey="dashboard.announcements.heading" />
        <p>{translate(locale, "dashboard.catalog.errorWrite")}</p>
      </>
    );
  }

  const row = await readAnnouncementRow(supabase, resolved.id);
  ...
  return (
    <>
      <DashboardModuleTitle locale={locale} titleKey="dashboard.announcements.heading" />
      <AnnouncementForm locale={locale} row={row} notice={notice} assets={assets} />
    </>
  );
}
